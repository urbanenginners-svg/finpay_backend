...
export class AdminForexOrdersController {
  ...
  @ApiOperation({
    ...
    description:
      'Updates status in Finpay MongoDB without calling the provider. Customer and agent bookings can pay from PENDING. Sends email/SMS when status changes (payment notify on DOCUMENTS_APPROVED_AWAITING_FUNDS).',
  })
